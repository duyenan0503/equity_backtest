# Equity Candlestick Pattern Backtest Strategy
...
| `backCandles` | 30 | Rolling window of past candles to scan for levels |

**Support** (`ss`): A candle whose `low` is a local minimum — i.e., each preceding candle has a lower low than its successor, and each following candle has a higher low than its predecessor (strict V-shape bottom).

**Resistance** (`rr`): A candle whose `high` is a local maximum — an inverted V-shape (Λ-shape top).

For every signal bar, the strategy collects **all active support prices** and **all active resistance prices** from the prior `backCandles` window.
...
Trades are executed at market open the candle **after** the signal fires.

| Parameter | Buy (Long) | Sell (Short) |
|---|---|---|
| Entry | Market order | Market order |
| Stop-Loss | `Close - 1.0 × ATR` | `Close + 1.0 × ATR` |
| Take-Profit | `Close + 1.5 × ATR` | `Close - 1.5 × ATR` |
| Risk:Reward | 1 : 1.5 | 1 : 1.5 |

No pyramiding — a new position is only opened when flat (`not self.position`).
...
- **Near-zero beta (−0.004)**: The strategy is essentially market-neutral in terms of directional market exposure, which shows good isolation of the alpha signal from broad market moves.
...
| **Negative Sharpe & Sortino** | Both risk-adjusted return metrics are negative, meaning the strategy generates losses even on a risk-adjusted basis. Any risk-free alternative (T-bills, money market) would have outperformed. |
...
This filters out counter-trend trades that are statistically less likely to succeed.

---

### 3. Strengthen S/R Level Quality

Not all pivot points carry equal weight. Prioritize S/R levels that have been **tested multiple times** (confluence zones):

```python
from collections import Counter

def cluster_levels(levels, tolerance):
    """Merge S/R levels within tolerance into clusters, keep only tested 2+ times."""
    clustered = []
    ...
    return [np.mean(c[1]) for c in clustered if len(c[1]) >= 2]  # min 2 touches
```
...
A minimum of **30–50 trades** is needed before any performance metric is meaningful.

---

### 5. Optimize Parameters with Walk-Forward Testing

Replace the fixed `backCandles=30`, `n1=2`, `n2=2` with a walk-forward optimization to avoid overfitting:
...
```python
# Risk 1% of equity per trade, sized by ATR-based stop distance
def next(self):
